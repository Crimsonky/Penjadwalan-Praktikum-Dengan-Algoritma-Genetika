...
function deleteDataBykodeRuang($conn, $kode_ruang) {
    $delete_query = "CALL hapus_ruangan(?)"; //Penerapan Stored Procedure (Bab1s)
    $stmt = mysqli_prepare($conn, $delete_query);
    mysqli_stmt_bind_param($stmt, 's', $kode_ruang);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
}

if (isset($_POST['delete_kode_ruang'])) {
    $kode_ruang_to_delete = $_POST['delete_kode_ruang'];
    deleteDataBykodeRuang($conn, $kode_ruang_to_delete);
}
?>
    ...
    <section class="main">
        ...
        <section class="main-course">
            <div class="course-box1">
                <a href="#" class="inputMatkul" onclick="tambahRuangan(); return false;">Tambah Ruangan</a>
...
<script>
    function tambahRuangan() {
        var addUrl = 'inputRuangan.php';
        loadContent(addUrl, true);
    }
</script>
<script>
    function editRuangan(kodeRuang) {
        var editUrl = 'editRuangan.php?kode_ruang=' + kodeRuang;
